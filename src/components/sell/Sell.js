...
const Sell = () => {
    ...
    try {
      ...
      if (isProductValid({ name, price, description })) {
        ...
        firebaseService.upload({
          key: FIREBASE_KEYS.PRODUCTS,
          id,
          payload: imageToProduct,
  ...
  const isProductValid = ({ name, price, description }) => {
    ...
    if (!validator.isNumeric(price)) {
      uiService.alert(
        'Please input your product price. Your product price must be a number'
      );
      return false;
    }
